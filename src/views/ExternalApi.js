...
export const ExternalApiComponent = () => {
  ...
  const callApi = async () => {
    try {
      ...
      const response = await fetch(`${apiOrigin}/role-based/protected/api/any`, {
        headers: {
  ...
  const callAdminApi = async () => {
    try {
      ...
      const response = await fetch(`${apiOrigin}/role-based/protected/api/admin`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const callSuperApi = async () => {
    try {
      const token = await getAccessTokenSilently();

      const response = await fetch(`${apiOrigin}/role-based/protected/api/super`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  let data = JSON.stringify(state.apiMessage, null, 2)
  return (
    <>
      <div className="mb-5">
